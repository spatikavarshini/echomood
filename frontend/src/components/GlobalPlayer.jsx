      ...
      <div className="max-w-6xl mx-auto px-4 py-3">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="min-w-0">
            <p className="text-[10px] tracking-[0.2em] uppercase text-zinc-500 mb-1">Now Playing</p>
            {hasTrack ? (
              <>
                <p className="text-sm text-white truncate">{currentTrack.track_name}</p>
                <p className="text-xs text-zinc-400 truncate">{currentTrack.artist_name}</p>
              </>
            ) : (
              <p className="text-sm text-zinc-400">Select a track from your Vault to play.</p>
            )}
          </div>
